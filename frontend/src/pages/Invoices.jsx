...
const STATUS_CONFIG = {
  ...
  paid: { label: 'Paid', bg: 'bg-green-900/30 text-green-400', dot: 'bg-green-500' },
  overdue: { label: 'Overdue', bg: 'bg-red-900/30 text-red-400', dot: 'bg-red-500' },
  ...
  return (
    <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50">
      <div className="bg-sand-100 rounded-lg shadow-lg p-6 w-full max-w-md border border-sand-200">
        <h2 className="text-base font-semibold text-navy-900 mb-4">Create Invoice</h2>
        <form onSubmit={e => { e.preventDefault(); mutation.mutate(form) }} className="space-y-3">
          <div>
            <label className="text-sm text-navy-600 block mb-1">Link to Deal</label>
            <select value={form.deal_id} onChange={onDealChange} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm bg-sand-50 text-sand-600">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-sm text-navy-600 block mb-1">Due Date</label>
              <input type="date" value={form.due_date} onChange={set('due_date')} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm bg-sand-50 text-sand-600" />
            </div>
          </div>
          <div>
            <label className="text-sm text-navy-600 block mb-1">Notes</label>
            <textarea value={form.notes} onChange={set('notes')} rows={2} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm resize-none bg-sand-50 text-sand-600" />
          </div>
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-semibold text-navy-900">Invoices</h1>
      ...
      {overdueAlerts.length > 0 && (
        <div className="mb-4 bg-red-900/30 border border-red-200 rounded-lg px-4 py-3 flex items-center gap-3">
          <span className="text-red-500 text-lg">⚠</span>
          <div>
            <p className="text-sm font-medium text-red-400">{overdueAlerts.length} invoice(s) past due date</p>
            <p className="text-xs text-red-400 mt-0.5">Consider marking them as overdue or following up with the brand</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="bg-sand-100 border border-sand-200 rounded-lg p-4 shadow-sm">
        ...
        <div className="bg-sand-100 border border-cyan-200 rounded-lg p-4 shadow-sm">
        ...
        <div className="bg-sand-100 border border-green-200 rounded-lg p-4 shadow-sm">
          <p className="text-xs text-green-400">Paid</p>
          <p className="text-lg font-semibold text-green-400 mt-1">{fmt(totalPaid)}</p>
          <p className="text-xs text-navy-300 mt-0.5">{invoices.filter(i => i.status === 'paid').length} invoices</p>
        </div>
        <div className="bg-sand-100 border border-red-200 rounded-lg p-4 shadow-sm">
          <p className="text-xs text-red-400">Overdue</p>
          <p className="text-lg font-semibold text-red-400 mt-1">{fmt(totalOverdue)}</p>
      ...
      <div className="bg-sand-100 border border-sand-200 rounded-lg shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-sand-200 flex items-center gap-3">
          <h2 className="text-sm font-medium text-navy-900 flex-1">All Invoices</h2>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-sand-200 rounded px-2 py-1 text-xs bg-sand-50 text-sand-600">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-sand-50">
            {isLoading ? (
              ...
              return (
                <tr key={inv.id} className={`hover:bg-sand-50 ${isPastDue ? 'bg-red-900/30' : ''}`}>
                  <td className="px-5 py-3">
                    <span className="text-sm font-mono text-navy-600">{inv.invoice_number}</span>
                  </td>
                  <td className="px-4 py-3">
                    {inv.deal ? (
                      <div>
                        <Link to={`/deals/${inv.deal.id}`} className="text-sm text-navy-900 hover:text-teal-600">{inv.deal.title}</Link>
                        {inv.deal.brand && <p className="text-xs text-navy-300">{inv.deal.brand.name}</p>}
                      </div>
                    ) : <span className="text-sm text-navy-300">—</span>}
                  </td>
                  <td className="px-4 py-3 text-sm font-semibold text-navy-900">{fmt(inv.amount)}</td>
                  <td className="px-4 py-3">
                    <span className={`text-sm ${isPastDue ? 'text-red-400 font-medium' : 'text-navy-500'}`}>
                    ...
                    <div className="flex items-center gap-1.5">
                      {inv.status === 'draft' && (
                        <button
                          ...
                          className="text-xs bg-cyan-50 text-cyan-700 px-2.5 py-1 rounded hover:bg-cyan-900/40 font-medium transition-colors"
                        >
                          Mark Sent
                        </button>
                      )}
                      {(inv.status === 'sent' || inv.status === 'overdue') && (
                        <button
                          ...
                          className="text-xs bg-green-900/30 text-green-400 px-2.5 py-1 rounded hover:bg-green-900/40 font-medium transition-colors"
                        >
                          Mark Paid
                        </button>
                      )}
                      {inv.status === 'sent' && isPastDue && (
                        <button
                          ...
                          className="text-xs bg-red-900/30 text-red-400 px-2.5 py-1 rounded hover:bg-red-900/40 font-medium transition-colors"
                        >
                          Mark Overdue
                        </button>
                      )}
                      {inv.status === 'draft' && (
                        <button
                          ...
                          className="text-xs text-navy-300 hover:text-red-400 px-1 transition-colors"
                        >
                          ✕
                        </button>
                      )}
      ...
      {invoices.length > 0 && (
        <div className="mt-5 bg-sand-100 border border-sand-200 rounded-lg shadow-sm p-5">
          <h2 className="text-sm font-medium text-navy-900 mb-3">Recent Activity</h2>
          <div className="space-y-2.5">
            {invoices.filter(i => i.paid_at || i.sent_at).slice(0, 5).map(inv => (
              <div key={inv.id} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-navy-600 font-mono text-xs">{inv.invoice_number}</span>
