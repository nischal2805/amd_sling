  ...
  return (
    <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md border border-sand-200">
        <h2 className="text-base font-semibold text-navy-800 mb-4">Add Brand</h2>
        <form onSubmit={e => { e.preventDefault(); mutation.mutate(form) }} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="text-sm text-navy-700 block mb-1">Brand name *</label>
              <input value={form.name} onChange={set('name')} required className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm focus:ring-1 focus:ring-teal-500 focus:border-teal-500" />
            </div>
            <div>
              <label className="text-sm text-navy-700 block mb-1">Industry</label>
              <input value={form.industry} onChange={set('industry')} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="text-sm text-navy-700 block mb-1">Website</label>
              <input value={form.website} onChange={set('website')} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="text-sm text-navy-700 block mb-1">Contact name</label>
              <input value={form.contact_name} onChange={set('contact_name')} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm" />
            </div>
            <div>
              <label className="text-sm text-navy-700 block mb-1">Contact email</label>
              <input type="email" value={form.contact_email} onChange={set('contact_email')} className="w-full border border-sand-200 rounded px-3 py-1.5 text-sm" />
            </div>
          </div>
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-semibold text-navy-800">Brands</h1>
              ...
              return (
                <tr key={brand.id} className="hover:bg-sand-50 transition-colors">
                  <td className="px-5 py-3">
                    <Link to={`/brands/${brand.id}`} className="text-sm font-medium text-navy-800 hover:text-teal-600">{brand.name}</Link>
                  ...
                  <td className="px-4 py-3 text-sm text-navy-800">{brand.total_deals || 0}</td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="text-xs text-navy-200 hover:text-red-500"
                    >Delete</button>
                  </td>
                </tr>
              )
